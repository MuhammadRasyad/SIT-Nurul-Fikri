<!-- resources/views/super-admin/data-admin-sd.blade.php -->
@extends('super-admin.nav-app')

@section('content')
<div class="container">
    <h1>Data Admin SDIT</h1>
    <button class="btn btn-primary" data-toggle="modal" data-target="#createAdminModal">Tambah Admin</button>

    <!-- Tabel Daftar Admin -->
    @if($adminSD->isEmpty())
        <div class="alert alert-warning mt-3">Tidak ada data Admin SDIT yang tersedia.</div>
    @else
                ...
                @foreach ($adminSD as $admin)
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="{{ route('super.admin.admin.sd.delete', $admin->id) }}" method="POST" style="display:inline;" class="delete-form">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="modal fade" id="editAdminModal{{ $admin->id }}" tabindex="-1" role="dialog" aria-labelledby="editAdminModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <form action="{{ route('super.admin.admin.sd.update', $admin->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="editAdminModalLabel">Edit Admin SDIT</h5>
    ...
    <div class="modal fade" id="createAdminModal" tabindex="-1" role="dialog" aria-labelledby="createAdminModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('super.admin.admin.sd.store') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="createAdminModalLabel">Tambah Admin SDIT</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
